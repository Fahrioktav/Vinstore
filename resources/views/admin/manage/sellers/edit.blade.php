...
@section('heroText', 'Edit Data Seller')

@section('content')
<div class="max-w-3xl mx-auto px-6 py-8">
    <div class="bg-white p-8 rounded-2xl shadow-md shadow-[#53685B]/20">
        <h2 class="text-3xl font-bold mb-6 text-[#53685B]">✏️ Edit Seller</h2>
        ...
        <form action="{{ route('admin.sellers.update', $seller->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <textarea name="address" id="address" rows="3"
                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('address') border-red-500 @enderror">{{ old('address', $seller->address) }}</textarea>
                @error('address')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
